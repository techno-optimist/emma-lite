...
export class VaultStorage {
  ...
  async processAttachmentsInTransaction(transaction, attachments, memoryId, vaultId) {
    ...
    for (const attachment of attachments) {
      try {
        console.log('🗄️ VaultStorage: Processing attachment:', {
          filename: attachment.filename,
          mimeType: attachment.mimeType,
          hasData: !!attachment.data,
          dataType: typeof attachment.data,
          dataPreview: attachment.data ? attachment.data.substring(0, 50) : 'null'
        });
        
        // Validate attachment data
        if (!attachment.data) {
          throw new Error(`Attachment ${attachment.filename || 'unknown'} has no data`);
        }
        ...
        console.error('🗄️ VaultStorage: Failed to process attachment in transaction:', {
          attachmentFile: attachment.filename || 'unknown',
          error: error.message,
          attachmentData: attachment.data ? {
            type: typeof attachment.data,
            length: attachment.data.length || 0,
            preview: attachment.data.substring(0, 100)
          } : 'missing'
        });
        throw new Error(`Failed to process attachment ${attachment.filename || 'unknown'}: ${error.message}`);
      }
    }
  }
  ...
  dataURLToBytes(dataURL) {
    // Handle undefined or invalid data URLs
    if (!dataURL || typeof dataURL !== 'string') {
      throw new Error('Invalid data URL: must be a valid string');
    }
    
    // Check if it's a proper data URL format
    if (!dataURL.includes(',')) {
      // Might be raw base64 - try to decode directly
      try {
    ...
    if (!data) {
      throw new Error(`Invalid data URL: missing data component in ${dataURL.substring(0, 50)}...`);
    }
    ...
    if (isBase64) {
      ...
      try {
        ...
        throw new Error(`Failed to decode URL data: ${e.message}`);
      }
    }
  }
