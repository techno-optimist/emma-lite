...
class VaultService {
  ...
  async execute(toolName, params = {}) {
    ...
    switch (name) {
      ...
      case 'get_related_memories':
        return this.getRelatedMemories(params);
      case 'delete_memory':
        return this.deleteMemory(params);
      case 'delete_person':
        return this.deletePerson(params);
      case 'get_memory_by_id':
        return this.getMemoryById(params);
      case 'get_person_by_id':
        return this.getPersonById(params);
      default:
  ...
  async deleteMemory({ memoryId }) {
    ...
    return this.updateVault((vault) => {
      const memoryIndex = vault.memories.findIndex((item) => item.id === memoryId);
      if (memoryIndex === -1) {
        throw new Error(`Memory ${memoryId} not found`);
      }

      const [deletedMemory] = vault.memories.splice(memoryIndex, 1);
      return {
        deleted: true,
  ...
  async deletePerson({ personId }) {
    ...
    return this.updateVault((vault) => {
      const personIndex = vault.people.findIndex((item) => item.id === personId);
      if (personIndex === -1) {
        throw new Error(`Person ${personId} not found`);
      }

      const [deletedPerson] = vault.people.splice(personIndex, 1);

      // Also remove the person from any memories
  ...
  async getMemoryById({ memoryId }) {
    if (!memoryId) {
      throw new Error('memoryId is required to get a memory by ID');
    }
  ...
  async getPersonById({ personId }) {
    if (!personId) {
      throw new Error('personId is required to get a person by ID');
    }

    const vault = await this.loadVault();
    const person = vault.people.find((item) => item.id === personId);
  ...
  async getMemories({ personId, dateRange, keyword, limit = 0 } = {}) {
    const vault = await this.loadVault();
    let results = vault.memories.slice();

    if (!personId && !dateRange && !keyword) {
      results = vault.memories;
    }
    ...
    return {
      ...
      memories: limit > 0 ? results.slice(0, limit) : results
    };
  }
  ...
  async getRelatedMemories({ memoryId, limit = 3 }) {
    if (!memoryId) {
      throw new Error('memoryId is required to get related memories');
    }

    const vault = await this.loadVault();
    const sourceMemory = vault.memories.find((item) => item.id === memoryId);

    if (!sourceMemory) {
      throw new Error(`Memory ${memoryId} not found`);
    }

    const sourceText = [sourceMemory.title, sourceMemory.content, ...(sourceMemory.tags || [])]
      .filter(Boolean)
      .join(' ')
      .toLowerCase();
    const sourceWords = new Set(sourceText.split(/\s+/).filter(word => word.length > 3));

    const related = vault.memories
      .filter((memory) => memory.id !== memoryId)
      .map((memory) => {
        const targetText = [memory.title, memory.content, ...(memory.tags || [])]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
        const targetWords = new Set(targetText.split(/\s+/));
        const intersection = new Set([...sourceWords].filter(word => targetWords.has(word)));
        return { memory, score: intersection.size };
      })
