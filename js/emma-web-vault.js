...
class EmmaWebVault {
  ...
  constructor() {
    ...
    if (vaultActive && vaultName) {
      ...
      setTimeout(async () => {
        ...
        this.maybePromptForAutoSave('activation');
  ...
  async requestDirectSaveHandle(context = 'auto-save-preference') {
    ...
    if (!this.fileHandle && this.canPersistFileHandles()) {
      ...
      try {
        ...
        const persisted = await this.loadPersistedFileHandle({ silent: true });

        if (persisted && persisted.handle) {

          this._suspendHandlePersistence = true;

          this.fileHandle = persisted.handle;

          this._suspendHandlePersistence = false;

          if (persisted.originalFileName && !this.originalFileName) {

            this.originalFileName = persisted.originalFileName;

          }

          if (persisted.permission === 'denied') {
        ...
        console.warn(' AUTO-SAVE HANDLE: Failed to restore persisted handle:', error);
  ...
  showAutoSavePreferencePrompt(context = 'activation') {
    ...
    const copy = (() => {
      switch (context) {
        case 'permission-lost':
          return {
            title: 'Restore Direct File Auto-Save',
            body: `Emma lost permission to update your <strong>.emma</strong> file after a browser restart or settings change.
              <br><br>
              Browser Auto Save is still active, but click below so the browser can grant edit access again and Emma will resume writing directly to your vault file.`
          };
        case 'manual-request':
          return {
            title: 'Turn On Direct Auto-Save?',
            body: `Browser Auto Save is already keeping your memories inside Emma Web Vault.
              <br><br>
              Enable <strong>Direct File Auto-Save</strong> if you'd like Emma to keep editing your .emma file on disk without downloading replacements.`
          };
        case 'legacy-upgrade':
          return {
            title: 'Upgrade This Vault to Direct Auto-Save',
            body: `This vault was created before Emma offered direct saving.
              <br><br>
              Keep Browser Auto Save if you prefer, or grant permission once so Emma can update your <strong>.emma</strong> file automatically while still safeguarding a browser copy.`
          };
        default:
          return {
            title: 'Choose Your Auto-Save Mode',
            body: `Browser Auto Save keeps every change encrypted inside this browser.
              <br><br>
              Turn on <strong>Direct File Auto-Save</strong> to have Emma write those updates straight into your .emma file after the browser grants edit permission.`
          };
      }
    })();
      ...
      ">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 8px;">
            ...
            <h2 style="margin: 0; font-size: 24px; color: #f8fafc;">${copy.title}</h2>
          ...
          <div style="display: flex; flex-direction: column; gap: 12px; margin: 6px 0 10px 0;">

            <div style="border: 1px solid rgba(148,163,184,0.35); border-radius: 16px; padding: 16px; background: rgba(148,163,184,0.08);">

              <div style="font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #cbd5f5; font-weight: 700;">Browser Auto Save (Default)</div>

              <p style="margin: 8px 0 0; font-size: 14px; line-height: 1.5; color: #e2e8f0;">Emma already keeps every change inside this browser so nothing is lost. Download the .emma file whenever you need a copy.</p>

            </div>

            <div style="border: 1px solid rgba(96,165,250,0.7); border-radius: 16px; padding: 16px; background: rgba(59,130,246,0.12);">

              <div style="font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #60a5fa; font-weight: 700;">Direct File Auto-Save</div>

              <p style="margin: 8px 0 0; font-size: 14px; line-height: 1.5; color: #dbeafe;">Give Emma one-time permission to edit your actual .emma file, so every change is written straight to the file without extra downloads.</p>
          ...
          <div style="display: flex; flex-direction: column; gap: 12px; margin-top: 8px;">
            ...
            ">Enable Direct Auto-Save</button>
            ...
            ">Stay in Browser Auto Save</button>
    ...
    const resetEnableButton = () => {
      ...
      enableButton.textContent = 'Enable Direct Auto-Save';
      ...
      try {
        ...
        if (restored && this.fileHandle) {
          ...
          if (this.showToast) {
            ...
            this.showToast(' Direct auto-save enabled. Emma will keep your .emma file updated.', 'success');

          }

          if (window.emmaSyncStatus) {

            window.emmaSyncStatus.show('success', 'Direct auto-save active - Emma will write directly to your .emma file');
    ...
    browserButton.addEventListener('click', () => {
      ...
      if (this.showToast) {
        ...
        this.showToast(' Browser Auto Save only. Use Download whenever you want a copy of your .emma file.', 'info');
  ...
  async loadPersistedFileHandle(options = {}) {
    ...
    try {
      ...
      if (!record.originalFileName && record.handle && record.handle.name) {

        record.originalFileName = record.handle.name;

      }

      const permission = await this.getHandlePermissionState(record.handle);
  ...
  canPersistFileHandles() {
    ...
    const hasHandleSupport = typeof window.FileSystemHandle !== 'undefined' || typeof window.FileSystemFileHandle !== 'undefined';

    return secureContext && hasIndexedDB && hasHandleSupport;
  ...
  async openHandleDatabase() {
    ...
    if (this._handleDbPromise) {

      return this._handleDbPromise;

    }



    this._handleDbPromise = new Promise((resolve, reject) => {

      const request = window.indexedDB.open('EmmaVaultHandles', 1);
      ...
      console.warn(' FILE-HANDLE: Unable to open handle database:', error);
  ...
  async persistFileHandle(handle) {

    if (!this.canPersistFileHandles() || !handle) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        store.put({
          ...
          originalFileName: this.originalFileName || null,
