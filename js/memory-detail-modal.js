...
/**
 * Open the elegant people selection modal - RESTORED FROM ORIGINAL SYSTEM
 */
async function openAddPeopleModalForGallery() {
  try {
    console.log('👥 MODAL: Opening elegant people selection modal');
    
    // Load all people from storage - try multiple sources like we do for reading
    let allPeople = [];
    
    // First try chrome storage (extension mode)
    if (typeof chrome !== 'undefined') {
      try {
        const store = await chrome.storage.local.get(['emma_people']);
        allPeople = Array.isArray(store.emma_people) ? store.emma_people : [];
        console.log('👥 MODAL: Loaded people from chrome storage:', allPeople.length);
      } catch (error) {
        console.warn('👥 MODAL: Could not access chrome storage:', error);
      }
    }
    
    // If no people from chrome storage, try web vault
    if (allPeople.length === 0 && window.emmaWebVault?.vaultData?.people) {
      const peopleObj = window.emmaWebVault.vaultData.people;
      allPeople = Object.values(peopleObj);
      console.log('👥 MODAL: Loaded people from web vault:', allPeople.length);
    }
    ...
    const currentlyTaggedPeople = currentMemory?.metadata?.people || [];
    const taggedPeopleIds = new Set(currentlyTaggedPeople.map(id => String(id)));
    ...
    // Create the elegant people selection modal
    const modal = document.createElement('div');
    ...
    // Create the beautiful people grid
    const peopleGridHtml = availablePeople.map(person => {
      const initials = (person.name || '?').split(' ').map(n => n[0]).join('').slice(0,2).toUpperCase();
      const avatar = person.profilePicture || person.avatarUrl
        ? \`<img src="\${person.profilePicture || person.avatarUrl}" style="width:60px; height:60px; object-fit:cover; border-radius:50%;"/>\`
    ...
    modal.innerHTML = \`
      <div class="memory-detail-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: transparent;"></div>
      <div class="memory-detail-content" style="position: relative; max-width: 800px; max-height: 80vh; background: rgba(26, 16, 51, 0.95); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 20px; color: white; overflow: hidden;">
        <div class="memory-detail-header" style="padding: 24px 32px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); display: flex; align-items: center; justify-content: space-between;">
          <div>
            <h2 style="margin: 0; font-size: 24px; font-weight: 600;">Add People to Memory</h2>
            <p style="margin: 8px 0 0; color: rgba(255, 255, 255, 0.7);">Select people who are connected to this memory</p>
          </div>
          <button class="modal-close-btn" style="background: none; border: none; color: white; font-size: 24px; cursor: pointer; padding: 8px;">&times;</button>
        </div>
        ...
        <div class="memory-detail-footer" style="padding: 24px 32px; border-top: 1px solid rgba(255, 255, 255, 0.1); display: flex; justify-content: space-between; align-items: center;">
          <div id="selected-count" style="color: rgba(255, 255, 255, 0.7);">0 people selected</div>
          <div style="display: flex; gap: 16px;">
            <button id="cancel-btn" class="btn btn-secondary" style="padding: 8px 16px; background: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2); color: white; border-radius: 8px; cursor: pointer;">Cancel</button>
            <button id="confirm-btn" class="btn btn-primary" style="padding: 8px 16px; background: linear-gradient(135deg, #8658ff, #f093fb); border: none; color: white; border-radius: 8px; cursor: pointer;" disabled>Add 0 People</button>
          </div>
        </div>
      </div>
    \`;
    ...
    // Set up the elegant selection system
    const selectedPeople = new Set();
    const confirmBtn = modal.querySelector('#confirm-btn');
    const cancelBtn = modal.querySelector('#cancel-btn');
    const selectedCountDiv = modal.querySelector('#selected-count');

    // Handle people selection
    modal.querySelectorAll('.selectable-person').forEach(personDiv => {
      personDiv.addEventListener('click', () => {
        ...
        if (selectedPeople.has(personId)) {
          ...
          personDiv.style.border = '2px solid transparent';
          personDiv.style.background = 'rgba(255,255,255,0.05)';
        } else {
          selectedPeople.add(personId);
          personDiv.style.border = '2px solid #8658ff';
          personDiv.style.background = 'rgba(134, 88, 255, 0.2)';
        }
        
        // Update UI
        const count = selectedPeople.size;
        selectedCountDiv.textContent = count === 1 ? '1 person selected' : \`\${count} people selected\`;
        confirmBtn.disabled = count === 0;
        confirmBtn.textContent = count === 0 ? 'Add 0 People' : \`Add \${count} People\`;
      });
    });
    ...
    // Handle confirm - THE CRITICAL CONNECTION PART
    confirmBtn.addEventListener('click', async () => {
      if (selectedPeople.size > 0) {
        try {
          ...
          // THIS IS THE KEY: Update the memory's metadata.people array
          const currentMemory = window.currentMemory;
          if (currentMemory) {
            if (!currentMemory.metadata) currentMemory.metadata = {};
            if (!currentMemory.metadata.people) currentMemory.metadata.people = [];
            
            // Add the new people IDs
            selectedPeopleList.forEach(person => {
              if (!currentMemory.metadata.people.includes(person.id)) {
                currentMemory.metadata.people.push(person.id);
              }
            });
            
            console.log('👥 MODAL: Updated memory metadata with people:', currentMemory.metadata.people);
          ...
          console.log(\`✅ Successfully tagged \${selectedPeopleList.length} people to this memory!\`);
          ...
          setTimeout(() => {
            const peopleModal = document.querySelector('.memory-modal');
            if (peopleModal && window.currentMemory) {
              loadPeopleContent(peopleModal, window.currentMemory);
            }
          }, 100);

        } catch (error) {
          console.error('Failed to tag people:', error);
          alert('Failed to tag people: ' + error.message);
    ...
    console.log('🎪 MODAL: Elegant people selection modal opened');
