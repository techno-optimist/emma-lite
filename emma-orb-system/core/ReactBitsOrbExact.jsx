 ...
 * 
 * Direct implementation of the ReactBits orb using WebGL
 * Based on the exact code from reactbits.dev
 */

import React, { useEffect, useRef } from "react";

export default function ReactBitsOrbExact({
  hue = 250,
  hoverIntensity = 0.2,
  rotateOnHover = true,
  forceHoverState = false,
  size = 80,
  settings = null // New prop for dynamic settings
}) {
  ...
  // Minimal WebGL setup without OGL
  ...
  useEffect(() => {
    // Apply dynamic settings if provided
    if (settings?.visual) {
      hue = settings.visual.hue || hue;
      hoverIntensity = settings.visual.haloIntensity || hoverIntensity;
    ...
    // Create triangle geometry (full screen)
    ...
    const update = (t) => {
      ...
      hoverValue += (effectiveHover - hoverValue) * 0.1;
  ...
  }, [hue, hoverIntensity, rotateOnHover, forceHoverState, size]);
