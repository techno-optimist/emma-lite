...
(function attachDashboardNavigationHandlers() {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    class EmmaDashboard {
      ...
      normalizePersonId(personLike) {
        if (!personLike) return null;

        if (typeof personLike === 'string' || typeof personLike === 'number') {
          const trimmed = String(personLike).trim();
          return trimmed || null;
        }

        if (typeof personLike === 'object') {
          const candidate = personLike.id ??
            personLike.personId ??
            personLike.personID ??
            personLike.person_id ??
            personLike.uuid ??
            personLike.guid;
          if (candidate === undefined || candidate === null) return null;
          const trimmed = String(candidate).trim();
          ...
          if (window.emmaWebVault && window.emmaWebVault.isOpen && window.emmaWebVault.vaultData) {
            ...
            const memories = Object.values(vaultMemories).map(memory => {
              ...
              return {
                ...
                // Use explicit people metadata when available to drive constellation links
                selectedPeople: Array.from(new Set([
                  ...(Array.isArray(memory.metadata?.people)
                    ? memory.metadata.people.map(person => this.normalizePersonId(person)).filter(Boolean)
                    : []),
                  ...(Array.isArray(memory.selectedPeople)
                    ? memory.selectedPeople.map(person => this.normalizePersonId(person)).filter(Boolean)
                    : [])
                ])),
      ...
      uid: personElement.dataset.nodeUid
        ...
        // Add central orb as a special node to anchor unlinked nodes (no random connections)
        this.centralNode = {
          x: centerX,
          y: centerY,
          element: this.orb,
        ...
        // Safely anchor isolated nodes to the orb instead of fabricating random links
        this.ensureConstellationAnchors();
      ...
      createMemoryPeopleConnections(memories, people) {
        ...
        const peopleNodes = this.nodes
          .filter(node => node.type === 'person')
          .map(node => {
            node.normalizedPersonId = this.normalizePersonId(node.person);
            return node;
          })
          .filter(node => !!node.normalizedPersonId);

        const peopleById = new Map();
        peopleNodes.forEach(node => {
          if (!peopleById.has(node.normalizedPersonId)) {
            peopleById.set(node.normalizedPersonId, []);
          }
          peopleById.get(node.normalizedPersonId).push(node);
        });
        ...
        memoryNodes.forEach(memoryNode => {
          ...
          const explicitIds = new Set(
            Array.isArray(memory.selectedPeople)
              ? memory.selectedPeople
                  .map(person => this.normalizePersonId(person))
                  .filter(Boolean)
              : []
          );

          if (Array.isArray(memory.metadata?.people)) {
            memory.metadata.people.forEach(personLike => {
              const id = this.normalizePersonId(personLike);
          ...
          explicitIds.forEach(id => {
            const nodes = peopleById.get(id);
            if (nodes && nodes.length > 0) {
              nodes.forEach(node => this.linkNodes(memoryNode, node));
              linked = true;
            }
          });

          if (!linked && peopleNodes.length > 0 && window.contentMentionsPerson) {
            const combinedText = [
              memory.title,
              memory.content,
              memory.description,
              ...(Array.isArray(memory.responses) ? memory.responses : [])
            ]
              .filter(Boolean)
              .join(' ');

            peopleNodes.forEach(personNode => {
              const personName = personNode.person?.name;
              if (personName && window.contentMentionsPerson(personName, combinedText)) {
      ...
      ensureConstellationAnchors() {
        if (!this.centralNode) return;
        const anchoredNodes = this.nodes.filter(node => !node.isCreateNode);
