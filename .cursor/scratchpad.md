...
- [x] Restrict extension permissions; remove `<all_urls>`; scope content scripts
...
- [x] Remove dev endpoints (http/ws localhost) from prod build
- [x] Eliminate localStorage fallback for sensitive data; enforce vault or explicit warning
- [x] Gate/strip debug logs; add runtime log level
- [x] Unify error handling UX; accessible, dementia-friendly messages
- [x] Accessibility and dementia UX pass (speech, delay, repetition, contrast)
- [x] Performance pass: budgets, lazy-load large pages, reduce motion
- [x] Remove stray files; tighten .gitignore for production
- [x] Final pre-demo test matrix and rollback plan executed
